...
| OpenCodex | `LUXERON/POLAR-PYRO-OPENCODEX-BRIDGE@440e7d856ec0e49e85b75ac88b75b40ce47f9d34`, inspired by `lidge-jun/opencodex@5840591322117f3ee9568b35b135a6d4339f7711` | Six adapter/HTTP tests pass. The exact Git pin installs through the DE-PIN lockfile. Live host qualification completed OFF → healthy ON → OFF; `/health` and `/v1/models` passed, and both OpenAI Responses and Anthropic Messages shapes are implemented in JSON/minimal SSE form. The bridge remains loopback-only and reports `execution_authority: false`. | **COMPATIBILITY-QUALIFIED / OFF BY DEFAULT.** Admission and lifecycle are green. Full provider-client matrices, cancellation fidelity, authenticated remote binds and workspace mutation remain gated. |
